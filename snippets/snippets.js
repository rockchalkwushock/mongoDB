...
MongoClient.connect('mongodb://localhost/snippets', function(err, db) {
    ...
    var read = function(name) {
      // query is optional but look at this like your validation check
      // to confirm we are querying the exact data we want.
        var query = {
            name: name
            // name: {$eq: name}  will check that name = passed name.
        };
    ...
    var update = function(name, content) {
        ...
        var update = {
            $set: { // $set: is an update operator find more at:
